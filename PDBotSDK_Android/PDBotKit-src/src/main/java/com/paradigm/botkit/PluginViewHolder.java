...
public class PluginViewHolder implements View.OnClickListener {
    ...
    public void hideItemAtIndex(int index, boolean hidden) {
        if (index < 0 || index >= pluginItems.size()) return;

        View itemLayout = pluginItems.get(index);
        itemLayout.setVisibility(hidden ? View.GONE : View.VISIBLE);
    }

    public void hideItemWithTag(int tag, boolean hidden) {
        int index = findItemWithTag(tag);
        hideItemAtIndex(index, hidden);
    }
